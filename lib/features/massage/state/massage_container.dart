...
class MassageContainer extends StatefulWidget {
  final List<MassageService> initialMassages;

  const MassageContainer({
    super.key,
    this.initialMassages = const [],
  });

  static Widget withMassages(List<MassageService> massages) {
    return MassageContainer(initialMassages: massages);
  }
...
class _MassageContainerState extends State<MassageContainer> {
  ...
  late List<MassageService> _massages;

  @override
  void initState() {
    super.initState();
    _massages = widget.initialMassages.isNotEmpty
        ? List.from(widget.initialMassages)
        : [
      MassageService(
        id: '1',
